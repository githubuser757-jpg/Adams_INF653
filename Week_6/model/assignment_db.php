...
function get_assignments_by_course($course_id)
{
    global $db;
    if ($course_id) {
        $query = 'SELECT assignments.ID, assignments.Description, courses.courseName FROM assignments
                  LEFT JOIN courses ON assignments.courseID = courses.courseID
                  WHERE assignments.courseID = :courseID ORDER BY assignments.ID';
    } else {
        $query = 'SELECT assignments.ID, assignments.Description, courses.courseName FROM assignments
                  LEFT JOIN courses ON assignments.courseID = courses.courseID ORDER BY courses.courseID';
    }
